...
function loadResults() {
    try {
        ...
        console.log("Score:", score); // Depuración
        ...
        if (!tbody) {
            throw new Error("Elemento tbody no encontrado");
        }
...
function showStoredNames() {
    ...
    if (storedNames.length > 0) {
        // Asegúrate de que el selector coincida con un elemento real en tu HTML
        const container = document.querySelector('.bg-green-500.text-center.py-2.mb-4');
        
        if (!container) {
            console.error('Contenedor no encontrado');
            return; // Sal de la función si el contenedor no se encuentra
        }
...
// Envuelve tu código en un controlador de eventos para asegurarte de que el DOM esté completamente cargado
document.addEventListener('DOMContentLoaded', (event) => {
    showStoredNames();
    loadUserName(); // Asume que esta función también necesita ser ejecutada después de que el DOM esté cargado
    loadResults();
});
